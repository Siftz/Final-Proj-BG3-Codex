...
public class ItemController(ItemRepository repo) : Controller
{
    ...
    // Displays all items with optional search this format worked better 
    public IActionResult Index(string search)
    {
        var items = repo.GetAllItems();
    ...
    public IActionResult AddToWardrobe(int id)
    {
        ...
        if (item != null)
        {
            var wardrobe = HttpContext.Session.GetObjectFromJson<List<Item>>("SelectedItems") ?? new List<Item>();
    ...
    {
        var selectedItems = HttpContext.Session.GetObjectFromJson<List<Item>>("SelectedItems") ?? new List<Item>();
    ...
    {
        var loadout = HttpContext.Session.GetObjectFromJson<CharacterLoadoutViewModel>("Loadout") ??
                      new CharacterLoadoutViewModel();
        var wardrobe = HttpContext.Session.GetObjectFromJson<List<Item>>("SelectedItems") ?? new List<Item>();
    ...
    public IActionResult EquipItem(int itemId)
    {
        ...
        var slot = NormalizeSlot(item.Type.TypeName);
        if (slot == null)
            return RedirectToAction("Loadout");

        // Check if item is two-handed
        var isTwoHanded = item.Property?.PropertyName?.StartsWith("2h") == true;
